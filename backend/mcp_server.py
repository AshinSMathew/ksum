...
class MCPServer:
    ...
    async def publish_event(self, source_agent: str, target_agent: str, event_type: str, patient_id: str, payload: Dict[str, Any]):
        ...
        # 1. Audit Log in Supabase (Attempt without hanging)
        try:
            # We use a short print for logging instead of a blocking insert if DB is slow
            print(f"[MCP LOG] Saving event to DB...")
            # For hackathon demo purposes, we don't want the UI/Server to hang if DB is slow
